...
@tool
def search_cpp_resources(query: str, skill_level: str = "beginner"):
    """
    Search for C++ programming information from trusted educational sources.
    ...
    try:
        # Base reliable C++ education sites
        base_sites = "site:en.cppreference.com OR site:isocpp.org OR site:cplusplus.com"

        # Add level-specific resources
        if skill_level.lower() == "beginner":
            level_sites = " OR site:learncpp.com OR site:geeksforgeeks.org OR site:w3schools.com/cpp OR site:codecademy.com OR site:freecodecamp.org"
        elif skill_level.lower() == "intermediate":
            level_sites = " OR site:learncpp.com OR site:geeksforgeeks.org OR site:medium.com OR site:dev.to OR site:cppbyexample.com"
        else:  # professional/advanced
            level_sites = " OR site:isocpp.org OR site:github.com OR site:stackoverflow.com OR site:cppcon.org OR site:accu.org"
        ...
        if "no results" in results.lower():
            ...
            return search.run(f"C++ programming {query}")
        return results

    except Exception as e:
        return f"Search error: {str(e)}. Try simpler terms like '{query.split()[0]} in C++'"