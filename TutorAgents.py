...
from TeachingTools import search_cpp_resources
from crewai import LLM

is_local = "localhost" in os.getenv("STREAMLIT_SERVER_BASE_URL", "localhost")
http_referer = "http://localhost:8501" if is_local else "https://cppautoteacher.streamlit.app/"
...
# Teaching Agent - Explains C++ concepts with extreme clarity
teaching_expert = Agent(
    role="C++ Concept Explainer",
    goal="Provide crystal clear explanations of C++ programming concepts tailored to the student's skill level with appropriate examples and analogies",
    backstory="""You are an award-winning computer science educator with 15 years of experience teaching C++ 
    to students of all backgrounds. You have a PhD in Computer Science Education and have authored three textbooks 
    on C++ programming. Your specialty is finding the perfect analogy to make complex concepts accessible to 
    ...
    tools=[],  # REMOVED search_cpp_resources to prevent tool errors
    verbose=True,
    max_iter=5,
    llm=llm,
    allow_delegation=False,
    description=(
        "SPECIALIZES IN: Breaking down C++ programming concepts into digestible parts with clear examples. "
...
code_reviewer = Agent(
    role="C++ Code Analyst and Debugger",
    goal="Thoroughly analyze C++ code, identify all issues (syntax errors, logic errors, style violations), provide specific fixes, and explain the reasoning behind improvements",
    backstory="""You are a senior software engineer at a major tech company with expertise in C++ code quality 
    and best practices. You've reviewed over 10,000 code submissions in your career and have a reputation for 
    providing the most helpful and constructive feedback. You follow C++ Core Guidelines religiously and believe 
    ...
    tools=[search_cpp_resources],
    verbose=True,
    max_iter=5,
    llm=llm,
    allow_delegation=False,
    description=(
        "SPECIALIZES IN: Code review, debugging, and suggesting improvements based on C++ best practices. "
        "REVIEW PROCESS: 1. Check for syntax errors 2. Identify logic errors 3. Evaluate code style and standards compliance "
        ...
        "SEARCH EXAMPLES: 'common C++ pointer errors and solutions', 'C++ Core Guidelines examples', "
        "'C++ exception handling best practices', 'how to optimize C++ loops'"
    )
)

# Curriculum Planner - Creates structured learning paths
curriculum_planner = Agent(
    role="C++ Learning Path Designer",
    ...
    You always consider the student's goals (systems programming, game development, embedded systems, etc.) when designing paths.""",
    tools=[search_cpp_resources],
    verbose=True,
    max_iter=5,
    llm=llm,
    ...
    description=(
        ...
        "SEARCH EXAMPLES: 'C++ learning path for systems programming', 'game development with C++ curriculum', "
        "'3-month C++ beginner to intermediate plan', 'project-based C++ learning schedule'"
    )
)

# Quiz Master - Creates and evaluates knowledge checks
quiz_master = Agent(
    role="C++ Knowledge Assessor",
    goal="Create effective quizzes to evaluate understanding of C++ concepts and provide detailed explanations for all answers to reinforce learning",
    ...
    allow_delegation=False,
    description=(
        "SPECIALIZES IN: Creating knowledge assessments that accurately measure understanding of C++ concepts. "
...
project_coordinator = Agent(
    ...
    for thousands of students. You have a deep understanding of C++ pedagogy and know exactly which specialist 
