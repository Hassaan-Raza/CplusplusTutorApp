...
# Page config & CSS
# -------------------------
st.set_page_config(page_title="C++ Tutor - AI C++ Learning Assistant", page_icon="🔷", layout="wide")
...
if "messages" not in st.session_state:
    ...
    try:
        saved_chat = st.session_state.local_storage.getItem("cplusplus_tutor_chat_history")
...
def save_chat(filename=CHAT_SAVE_FILENAME):
    ...
    try:
        ...
        st.session_state.local_storage.setItem("cplusplus_tutor_chat_history",
...
def format_code_blocks(text):
    ...
    code_blocks = re.findall(r'```(?:cpp)?\s*(.*?)\s*```', text, re.DOTALL)
...
def get_chat_context():
    """Get the conversation context for the AI to understand previous messages."""
...
with col1:
    ...
    <div class="header-container">
        <div class="header">🔷 C++ Tutor</div>
        <div class="sub">Your AI C++ Learning Assistant — from beginner to professional</div>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    if st.button("Reset Chat", type="secondary"):
        st.session_state.messages = []
        st.session_state.local_storage.deleteItem("cplusplus_tutor_chat_history")
...
st.markdown("### How to use C++ Tutor:")
st.markdown("""
- **Introduce yourself** - Tell me about your C++ experience level
- **Ask for explanations** - "Explain pointers in C++" or "What are classes?"
- **Get code reviews** - Paste your C++ code and ask for feedback
- **Request learning paths** - "Create a learning plan for game development"
- **Take quizzes** - "Test me on inheritance" or "Give me a quiz about templates"
""")

# Put initial assistant greeting if no messages exist
if not st.session_state.messages:
    greeting = (
        "Hello! I'm C++ Tutor, your AI C++ learning assistant. "
        "Tell me about your current experience with C++ (beginner/intermediate/advanced), "
        "and what you'd like to achieve (game development, systems programming, embedded systems, etc.)."
    )
...
def process_user_input_and_run(user_input: str) -> str:
    ...
    if not user_input or len(user_input.strip()) < 2:
        return "I'm here to help you learn C++! What would you like to know about?"
    ...
    if is_conversational or is_short_message:
        ...
        base_task = conversation_task(user_input, context=context)
    ...
    # ===== TEACHING INTENT =====
    teaching_phrases = [
        "teach me", "explain", "what is", "how to", "learn c++",
    ...
    if teaching_requested:
        ...
        topic = user_input if len(user_input.split()) < 30 else "C++ programming from beginner to advanced"
        base_task = teaching_task(topic, skill, context=context, student_background="")
        desc, expected = task_to_strings(base_task)
        assigned_task = Task(
            description=desc,
            expected_output=expected or base_task.expected_output,
    ...
    # Enhanced C++ code detection
    has_cpp_code = False
    cpp_indicators = [
        "#include", "using namespace", "int main", "cout", "cin", "endl",
        "class ", "struct ", "template ", "typename ", "auto ", "constexpr ",
        "virtual ", "override ", "final ", "public:", "private:", "protected:",
        "new ", "delete ", "nullptr", "try", "catch", "throw", "noexcept",
        "std::", "vector<", "string ", "map<", "set<", "unique_ptr", "shared_ptr",
        "for ", "while ", "do ", "if ", "else ", "switch ", "case ", "default ",
        "return ", "void ", "int ", "float ", "double ", "char ", "bool ", "unsigned ",
        "const ", "static ", "extern ", "volatile ", "mutable ", "explicit ",
        "operator", "friend ", "typedef ", "enum ", "union ", "namespace "
    ]
    ...
    # Also check for common C++ patterns
    if not has_cpp_code:
        if (re.search(r'[a-zA-Z_][a-zA-Z0-9_]*\s*=', user_input) or  # variable assignment
                ...
                re.search(r'/\*.*?\*/', user_input, re.DOTALL) or  # multi-line comments
                re.search(r'(if|for|while|do|switch|case|class|struct|template|namespace)\s+[a-zA-Z_]',
                          user_input)):  # control flow
            has_cpp_code = True

    # Check for code blocks with backticks
    has_code_block = re.search(r"```(?:cpp)?\s*(.*?)\s*```", user_input, re.DOTALL | re.IGNORECASE)

    # If user is asking for code review or has C++ code with a question, route to code reviewer
    if code_review_requested or (has_cpp_code and any(q in lower for q in ["?", "what", "why", "how", "help"])):
        # Extract code - use the entire input if no code block is found
        code = has_code_block.group(1) if has_code_block else user_input

        base_task = code_review_task(code, skill, context=context)
        desc, expected = task_to_strings(base_task)
        assigned_task = Task(
            description=desc,
            expected_output=expected or base_task.expected_output,
    ...
    curriculum_phrases = [
        ...
        "where to start", "how to learn c++", "study guide",
    ...
    if any(p in lower for p in quiz_phrases):
        ...
        for cue in ("about", "on"):
            if cue in lower:
                ...
                break
        base_task = quiz_task(topic, skill, context=context)
        desc, expected = task_to_strings(base_task)
        assigned_task = Task(
            description=desc,
            expected_output=expected or base_task.expected_output,
    ...
    base_task = coordination_task(context, user_input, skill, goals)
    desc, expected = task_to_strings(base_task)
    assigned_task = Task(
        description=desc,
        expected_output=expected or base_task.expected_output,
...
user_input = st.chat_input(
    "Tell me about your C++ experience or ask for a lesson...",
...
st.markdown("""
<div class="footer">
    <p>C++ Tutor - AI C++ Learning Assistant | Built with CrewAI & Streamlit | 🇵🇸 Free Palestine</p>
</div>
""", unsafe_allow_html=True)